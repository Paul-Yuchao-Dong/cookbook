class Node:
    ...
    def bfs(self, goal):
        ...
        frontier, i, level, parent = {self}, 0, defaultdict(set), {self:None}
        level[0].add(self) # initialization of BFS search
        while frontier:
            next_ = set()
            for explore in frontier:
                for next_node, way, dist in explore.neighbors():
                    if next_node not in parent: # filter out the already explired nodes
                        parent[next_node] = explore, way
                        level[i+1].add(next_node)
                        next_.add(next_node)
                        if next_node == goal: # hit the escape
                            path = []
                            while next_node is not self:
                                next_node, way = parent[next_node]
                                path.append(way)
                            path.reverse()
                            return path
            frontier = next_
